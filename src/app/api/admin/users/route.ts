...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (userId === session.user.id && role !== "SUPER_ADMIN") {
      return NextResponse.json({ error: "You cannot demote your own super admin session here" }, { status: 400 });
    }
    ...
    if (role !== "SUPER_ADMIN") {
      ...
      if (targetUser?.role === "SUPER_ADMIN" && superAdminCount <= 1) {
        return NextResponse.json({ error: "The last SUPER_ADMIN cannot be demoted" }, { status: 400 });
      }
    }
