...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const formattedItems = cart.items.map(item => {
            const basePrice = item.product.isOnSale && item.product.salePrice
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const queryVariantId: string | null = variantId ?? null;
        ...
        logger.error("Add to cart error", error as Error, { context: "cart-api-post", userId: session?.user?.id, productId });
...
export async function DELETE(request: NextRequest) {
    ...
    try {
        ...
        logger.error("Clear cart error", error as Error, { context: "cart-api-delete", userId: session?.user?.id });
        return NextResponse.json(
