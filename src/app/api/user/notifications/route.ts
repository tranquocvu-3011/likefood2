...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        logger.error('Get notifications error', error as Error, { context: 'user-notifications-api-get', userId: session?.user?.id });
        return NextResponse.json(
...
export async function PATCH(request: NextRequest) {
    ...
    try {
        ...
        logger.error('Mark notification error', error as Error, { context: 'user-notifications-api-patch', userId: session?.user?.id });
        return NextResponse.json(
            { error: 'Có lỗi xảy ra' },
            { status: 500 }
        );
...
export async function DELETE(request: NextRequest) {
    ...
    try {
        ...
        logger.error('Delete notification error', error as Error, { context: 'user-notifications-api-delete', userId: session?.user?.id });
        return NextResponse.json(
            { error: 'Có lỗi xảy ra' },
            { status: 500 }
        );
