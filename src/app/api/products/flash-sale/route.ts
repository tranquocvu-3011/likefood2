    ...
    try {
        ...
        for (const campaign of activeCampaigns) {
            for (const fp of campaign.products) {
                if (fp.product.inventory > 0 && !campaignMap.has(fp.productId)) {
                    campaignMap.set(fp.productId, {
        ...
        const merged: Array<{
            id: string; slug: string; name: string; originalPrice: number;
        ...
        for (const p of saleProducts) {
            seenIds.add(p.id);
            const salePrice = campaignMap.get(p.id)?.flashSalePrice ?? p.salePrice;
            const endAt = campaignMap.get(p.id)?.endAt ?? p.saleEndAt;
