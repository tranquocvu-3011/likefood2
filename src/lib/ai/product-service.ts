...
function formatVariantName(variant: productvariant): string {
  return [variant.flavor, variant.weight].filter(Boolean).join(" - ") || variant.sku || variant.id;
}
...
export async function getRelatedProducts(productId: string, limit = 4): Promise<ProductInfo[]> {
  try {
