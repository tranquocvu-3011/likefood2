...
const DEFAULT_KNOWLEDGE: Array<Omit<KnowledgeItem, "id" | "isActive">> = [
  ...
  { category: "shipping", question: "Có nhận hàng tại cửa hàng không?", answer: "Hiện tại LIKEFOOD chỉ phục vụ giao hàng tận nơi. Bạn có thể đến showroom để xem sản phẩm trực tiếp (nếu có).", keywords: ["nhan tai cua hang", "pickup", "store pickup"], language: "vi", priority: 7 },
  ...
  { category: "general", question: "Có cửa hàng (showroom) không?", answer: "Hiện LIKEFOOD chủ yếu online. Có thể đến showroom tại một số thành phố lớn (NYC, LA, Houston). Xem địa chỉ trên website.", keywords: ["showroom", "cua hang", "store"], language: "vi", priority: 8 },
  ...
  { category: "general", question: "Do you have a physical store?", answer: "Currently LIKEFOOD is mainly online. Some showrooms available in major cities (NYC, LA, Houston). Check website for addresses.", keywords: ["store", "showroom", "physical location"], language: "en", priority: 8 },
...
export async function getKnowledgeByCategory(
  ...
  try {
    ...
    console.error("getKnowledgeByCategory error:", error);
    return [];
  }
}
...
export async function seedKnowledgeBase(): Promise<void> {
  try {
    ...
    if (existingCount > 0) {
      console.log(`Knowledge base already has ${existingCount} items. Skipping seed.`);
      return;
    }
    console.log(`Seeding ${DEFAULT_KNOWLEDGE.length} knowledge items...`);
    ...
    console.log(`Seeded ${DEFAULT_KNOWLEDGE.length} knowledge items successfully.`);
  } catch (error) {
