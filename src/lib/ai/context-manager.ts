...
const viewedProductsStore = new Map<string, string[]>();
...
export async function getRecentlyViewedProducts(sessionId: string): Promise<string[]> {
  ...
  return viewedProductsStore.get(key) ?? [];
}
...
export async function trackProductView(sessionId: string, productId: string): Promise<void> {
  ...
  const current = viewedProductsStore.get(key) ?? [];
  viewedProductsStore.set(key, [productId, ...current].slice(0, 10));
}
