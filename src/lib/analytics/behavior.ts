...
export async function trackEvent(event: Omit<BehaviorEvent, "id" | "createdAt">): Promise<string> {
  try {
...
export async function calculateUserSegment(userId: string): Promise<string[]> {
  const events = await prisma.behaviorEvent.findMany({
    where: { userId },
    orderBy: { createdAt: "desc" },
    take: 100,
