...
export async function createCashoutRequest(
  ...
  return { success: true, cashoutId: cashout.id };
}

/**
 * Process store credit conversion (immediate)
 */
async function processStoreCredit(userId: string, amount: number): Promise<CashoutResult> {
  const debited = await debitAvailableBalance(
    userId,
    amount,
 ...
 */
async function processVoucherConversion(userId: string, amount: number): Promise<CashoutResult> {
  const debited = await debitAvailableBalance(
    userId,
    amount,
  ...
  const coupon = await prisma.coupon.create({
    data: {
      code: `REF-${userId.slice(-4)}-${Date.now().toString(36).toUpperCase()}`,
      discountType: "fixed",
