...
export function CouponSection({
    ...
    const handleApplyCoupon = async () => {
        if (!couponCode.trim()) {
            toast.error(language === "vi" ? 'Vui lòng nhập mã giảm giá' : 'Please enter a coupon code');
            return;
        }
        ...
        try {
            ...
            if (response.ok && data.valid) {
                ...
                toast.success(data.message || (language === "vi" ? 'Áp dụng mã giảm giá thành công!' : 'Coupon applied successfully!'));
            } else {
                toast.error(data.error || (language === "vi" ? 'Mã giảm giá không hợp lệ' : 'Invalid coupon code'));
                setCouponApplied(false);
                setCouponDiscount(0);
            }
        } catch (error) {
            logger.error('Coupon error', error as Error, { context: 'cart-page' });
            toast.error(language === "vi" ? 'Có lỗi xảy ra khi áp dụng mã giảm giá' : 'Error applying coupon');
        } finally {
    ...
    const handleRemoveCoupon = () => {
        ...
        setCouponCode('');
        setAppliedCouponCode('');
        toast.success(language === "vi" ? 'Đã xóa mã giảm giá' : 'Coupon removed');
    };
            ...
            {couponApplied ? (
                ...
                <div className="flex gap-2">
                    <input
                        ...
                        placeholder={language === "vi" ? "Nhập mã giảm giá" : "Enter coupon code"}
