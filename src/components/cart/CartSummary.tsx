        ...
        <div className="lg:col-span-4 space-y-6">
                ...
                <div className="flex items-center gap-3 mb-3">
                    ...
                    {progress >= 100 ? (
                        <p className="text-sm font-bold text-green-600">
                            🎉 {language === "vi" ? "Bạn đã đủ điều kiện" : "You qualify for"} <span className="font-black uppercase">{t("cart.freeShipping")}!</span>
                        </p>
                    ) : (
                        <p className="text-sm font-bold text-slate-600">
                            {language === "vi" ? "Mua thêm" : "Add"} <span className="font-black text-primary">{formatPrice(remaining)}</span> {language === "vi" ? "để được" : "for"} <span className="font-black text-green-600 uppercase">{t("cart.freeShipping")}</span>
                        </p>
                    )}
                </div>
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm">
                ...
                <div className="mt-6 p-4 bg-slate-50 rounded-2xl">
                    <p className="text-xs font-bold text-slate-400 uppercase mb-1">{language === "vi" ? "Dự kiến giao hàng" : "Estimated delivery"}</p>
                ...
                <Link href="/checkout" className="block mt-6">
                    ...
                    >
                        {hasOutOfStockItems
                            ? (language === "vi" ? "Có sản phẩm hết hàng" : "Out of stock items")
                            : t("cart.checkout")
                        }
                    </Button>
                </Link>
