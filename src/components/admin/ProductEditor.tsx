...
export default function ProductEditor({
  ...
  const buildPayload = (): ProductSubmitPayload | null => {
    ...
    if (Number.isNaN(parsedInventory) || parsedInventory < 0) {
      toast.error("Inventory must be zero or greater.");
      return null;
    }
  ...
  const handleGenerateDescription = async () => {
    if (!formData.name.trim() || !formData.category) {
      toast.error("Add the product name and category before using AI.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) throw new Error(data?.error || "Unable to generate AI content right now.");
      ...
      toast.success("AI drafted new content for this product.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Unable to generate AI content.");
    } finally {
  ...
  const addVariant = async () => {
    ...
    try {
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data?.error || "Unable to add variant.");
      setVariants((prev) => [data, ...prev]);
      setVariantDraft(EMPTY_VARIANT);
      toast.success("Variant created.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Unable to add variant.");
    } finally {
  ...
  const saveVariant = async (variantId: string) => {
    ...
    try {
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data?.error || "Unable to update variant.");
      setVariants((prev) => prev.map((variant) => (variant.id === variantId ? data : variant)));
      setEditingVariantId(null);
      toast.success("Variant updated.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Unable to update variant.");
    } finally {
      setVariantBusyId(null);
    }
  };
  ...
  const deleteVariant = async (variantId: string) => {
    ...
    try {
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data?.error || "Unable to remove variant.");
      setVariants((prev) => prev.filter((variant) => variant.id !== variantId));
      toast.success(data?.message || "Variant removed.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Unable to remove variant.");
    } finally {
      setVariantBusyId(null);
    }
  };
    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h1 className="mt-4 text-4xl font-black tracking-tight text-slate-950">{mode === "create" ? "Create product" : "Edit product"}</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
            Build a product record that is easy to merchandise, easy to price, and easy for the team to maintain.
          </p>
        </div>
      ...
      <form id="product-editor-form" onSubmit={handleSubmit} className="grid gap-8 xl:grid-cols-[1.08fr_0.92fr]">
        <div className="space-y-8">
          <Card className="rounded-[2.25rem] border-slate-200 bg-white shadow-sm">
            <CardContent className="space-y-6 p-6 lg:p-8">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-[11px] font-black uppercase tracking-[0.22em] text-slate-400">Core details</p>
                  <h2 className="mt-2 text-2xl font-black tracking-tight text-slate-950">Product information</h2>
                </div>
                ...
                <Field label="Description" required className="md:col-span-2">
                  <div className="space-y-3">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <p className="text-xs font-medium text-slate-500">Keep this operator-friendly: origin, flavor, use cases, storage, and why shoppers should buy it.</p>
                    ...
                    <textarea rows={8} value={formData.description} onChange={(event) => updateField("description", event.target.value)} className="admin-textarea" placeholder="Describe what makes the product distinctive, who it is for, and how it should be positioned." />
                    <textarea rows={3} value={aiBrief} onChange={(event) => setAiBrief(event.target.value)} className="admin-textarea text-sm" placeholder="Optional AI brief: origin, intended audience, flavor notes, gift context, best use case..." />
                  </div>
                </Field>
              </div>
              ...
              {aiInsights ? (
                <div className="rounded-[1.75rem] border border-emerald-200 bg-emerald-50 p-5">
                  <div className="flex items-center justify-between gap-3">
                    <div className="inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.22em] text-emerald-700"><Sparkles className="h-3.5 w-3.5" />AI recommendations</div>
                  ...
                  <div className="mt-4 grid gap-4 lg:grid-cols-2">
                    <InsightBlock icon={MapPin} label="Origin" value={aiInsights.origin} />
                    <InsightBlock icon={ChefHat} label="Use case" value={aiInsights.howToUse} />
                  </div>
          ...
          <Card className="rounded-[2.25rem] border-slate-200 bg-white shadow-sm">
            <CardContent className="space-y-6 p-6 lg:p-8">
              <div>
                <p className="text-[11px] font-black uppercase tracking-[0.22em] text-slate-400">Pricing and stock</p>
                <h2 className="mt-2 text-2xl font-black tracking-tight text-slate-950">Commerce controls</h2>
              </div>
              <div className="grid gap-5 md:grid-cols-2">
                <Field label="Base price (USD)" required><input type="number" min="0" step="0.01" value={formData.price} onChange={(event) => updateField("price", event.target.value)} className="admin-input" placeholder="29.00" /></Field>
                <Field label="Inventory" required><input type="number" min="0" step="1" value={formData.inventory} onChange={(event) => updateField("inventory", event.target.value)} className="admin-input" placeholder="100" /></Field>
                <Field label="Original price (optional)"><input type="number" min="0" step="0.01" value={formData.originalPrice} onChange={(event) => updateField("originalPrice", event.target.value)} className="admin-input" placeholder="35.00" /></Field>
                <Field label="Sale price (optional)"><input type="number" min="0" step="0.01" value={formData.salePrice} onChange={(event) => updateField("salePrice", event.target.value)} className="admin-input" placeholder="25.00" /></Field>
              </div>
              <div className="grid gap-4 lg:grid-cols-3">
                <MetricTile label="Base price" value={formatPrice(basePrice)} />
                <MetricTile label="Sale status" value={formData.isOnSale ? "Enabled" : "Off"} />
                <MetricTile label="Tag count" value={`${tagCount}`} />
              </div>
              <div className="grid gap-4 md:grid-cols-2">
                <ToggleRow title="Mark as featured" description="Feature the product on high-visibility modules." checked={formData.featured} onChange={(checked) => updateField("featured", checked)} />
                <ToggleRow title="Show sale treatment" description="If enabled and a sale price exists, the product shows promotional pricing." checked={formData.isOnSale} onChange={(checked) => updateField("isOnSale", checked)} />
              </div>
            </CardContent>
          </Card>
            ...
            <Card className="rounded-[2.25rem] border-slate-200 bg-white shadow-sm">
              <CardContent className="space-y-6 p-6 lg:p-8">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    <p className="text-[11px] font-black uppercase tracking-[0.22em] text-slate-400">Variants</p>
                    <h2 className="mt-2 text-2xl font-black tracking-tight text-slate-950">Variant manager</h2>
                  </div>
                  <div className="rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-slate-500">{variants.length} variants</div>
                </div>

                <div className="grid gap-4 rounded-[1.75rem] border border-slate-200 bg-slate-50 p-4 md:grid-cols-2 xl:grid-cols-6">
                  <MiniInput label="Weight" value={variantDraft.weight} onChange={(value) => setVariantDraft((prev) => ({ ...prev, weight: value }))} placeholder="500g" />
                  <MiniInput label="Flavor" value={variantDraft.flavor} onChange={(value) => setVariantDraft((prev) => ({ ...prev, flavor: value }))} placeholder="Original" />
                  <MiniInput label="Price delta" value={variantDraft.priceAdjustment} onChange={(value) => setVariantDraft((prev) => ({ ...prev, priceAdjustment: value }))} placeholder="0" type="number" />
                  <MiniInput label="Stock" value={variantDraft.stock} onChange={(value) => setVariantDraft((prev) => ({ ...prev, stock: value }))} placeholder="0" type="number" />
                ...
                {variants.length === 0 ? (
                  <div className="rounded-[1.5rem] border border-dashed border-slate-200 bg-slate-50 p-6 text-sm leading-6 text-slate-500">Add variants when a product needs multiple pack sizes, flavors, or price tiers.</div>
                ) : (
                  <div className="space-y-3">
                        ...
                        <div key={variant.id} className="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm">
                          {isEditing ? (
                            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
                              <MiniInput label="Weight" value={editingVariant.weight} onChange={(value) => setEditingVariant((prev) => ({ ...prev, weight: value }))} placeholder="500g" />
                              <MiniInput label="Flavor" value={editingVariant.flavor} onChange={(value) => setEditingVariant((prev) => ({ ...prev, flavor: value }))} placeholder="Original" />
                              <MiniInput label="Price delta" value={editingVariant.priceAdjustment} onChange={(value) => setEditingVariant((prev) => ({ ...prev, priceAdjustment: value }))} placeholder="0" type="number" />
                              <MiniInput label="Stock" value={editingVariant.stock} onChange={(value) => setEditingVariant((prev) => ({ ...prev, stock: value }))} placeholder="0" type="number" />
                              ...
                              <div className="flex items-end gap-2">
                                <Button type="button" size="sm" onClick={() => saveVariant(variant.id)} disabled={busy} className="flex-1">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}Save</Button>
                                <Button type="button" size="sm" variant="outline" onClick={() => setEditingVariantId(null)}>Cancel</Button>
                              </div>
                            </div>
                          ) : (
                            <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
                                <VariantStat label="Weight" value={variant.weight || "-"} />
                                <VariantStat label="Flavor" value={variant.flavor || "-"} />
                                <VariantStat label="Price delta" value={formatPrice(variant.priceAdjustment)} />
                                <VariantStat label="Stock" value={`${variant.stock}`} />
                                <VariantStat label="SKU" value={variant.sku || "-"} />
                              </div>
                              <div className="flex flex-wrap items-center gap-2">
                                <span className={`rounded-full px-3 py-2 text-xs font-black uppercase tracking-[0.16em] ${variant.isActive ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-600"}`}>{variant.isActive ? "Active" : "Hidden"}</span>
                                <Button type="button" size="sm" variant="outline" onClick={() => { setEditingVariantId(variant.id); setEditingVariant({ weight: variant.weight || "", flavor: variant.flavor || "", priceAdjustment: String(variant.priceAdjustment ?? 0), stock: String(variant.stock ?? 0), sku: variant.sku || "", isActive: variant.isActive }); }}>Edit</Button>
                                <Button type="button" size="sm" variant="outline" onClick={() => deleteVariant(variant.id)} disabled={busy}>{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}Remove</Button>
                              </div>
                            </div>
                          )}
                        </div>
        ...
        <div className="space-y-8 xl:sticky xl:top-8 xl:self-start">
          <Card className="rounded-[2.25rem] border-slate-200 bg-white shadow-sm">
            <CardContent className="space-y-6 p-6 lg:p-8">
              <div>
                <p className="text-[11px] font-black uppercase tracking-[0.22em] text-slate-400">Merchandising</p>
                <h2 className="mt-2 text-2xl font-black tracking-tight text-slate-950">Badges and discovery</h2>
              </div>
              <Field label="Badge text"><input value={formData.badgeText} onChange={(event) => updateField("badgeText", event.target.value)} className="admin-input" placeholder="Best seller, New, Limited" /></Field>
              <Field label="Tags"><textarea rows={4} value={formData.tags} onChange={(event) => updateField("tags", event.target.value)} className="admin-textarea" placeholder="giftable, premium, ready-to-serve" /></Field>
              <div className="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-slate-600">
                <p className="font-black text-slate-900">Current preview</p>
                <p className="mt-2">Category: {getAdminCategoryLabel(formData.category)}</p>
                <p>Inventory: {formData.inventory || "0"}</p>
                <p>Featured: {formData.featured ? "Yes" : "No"}</p>
                <p>On sale: {formData.isOnSale ? "Yes" : "No"}</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-[2.25rem] border-slate-200 bg-white shadow-sm">
            <CardContent className="space-y-6 p-6 lg:p-8">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-black tracking-tight text-slate-950">Image management</h2>
              </div>
              <Field label="Primary image"><input value={formData.image} onChange={(event) => updateField("image", event.target.value)} className="admin-input" placeholder="Primary image URL" /></Field>
              <div className="space-y-3">
                <p className="text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">Gallery</p>
