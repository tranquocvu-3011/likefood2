            ...
            <CardContent className="space-y-6 p-6 lg:p-8">
              ...
              <div className="grid gap-5 md:grid-cols-2">
                <Field label="Tên sản phẩm" required className="md:col-span-2">
                  <input value={formData.name} onChange={(event) => updateField("name", event.target.value)} className="w-full h-14 rounded-2xl border border-zinc-700 bg-zinc-900 px-5 text-sm font-semibold text-zinc-100 outline-none transition-colors focus:border-emerald-500/50 focus:bg-zinc-800" placeholder="Example: Kho ca loc premium" />
                </Field>
                ...
                <Field label="Quy cách / trọng lượng">
                  <input value={formData.weight} onChange={(event) => updateField("weight", event.target.value)} className="w-full h-14 rounded-2xl border border-zinc-700 bg-zinc-900 px-5 text-sm font-semibold text-zinc-100 outline-none transition-colors focus:border-emerald-500/50 focus:bg-zinc-800" placeholder="500g, 1kg, gift box" />
                </Field>
          ...
          {productId ? (
            <Card className="rounded-[2.25rem] border border-zinc-800 bg-[#111113]">
              <CardContent className="space-y-6 p-6 lg:p-8">
                ...
                <div className="grid gap-4 rounded-[1.75rem] border border-zinc-800 bg-zinc-900/50 p-4 md:grid-cols-2 xl:grid-cols-6">
                ...
                {variants.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {variants.map((variant) => {
                      ...
                      return (
                        <div key={variant.id} className="rounded-[1.5rem] border border-zinc-800 bg-[#111113] p-4">
                          {isEditing ? (
        ...
        <div className="space-y-8 xl:sticky xl:top-8 xl:self-start">
          <Card className="rounded-[2.25rem] border border-zinc-800 bg-[#111113]">
            <CardContent className="space-y-6 p-6 lg:p-8">
              ...
              <div className="rounded-[1.5rem] border border-zinc-800 bg-zinc-900/50 p-4 text-sm leading-6 text-zinc-400">
...
function InsightBlock({ icon: Icon, label, value }: { icon: typeof MapPin; label: string; value: string }) {
  ...
  return <div className="rounded-[1.35rem] border border-zinc-800 bg-zinc-900/50 p-4"><div className="inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.18em] text-zinc-500"><Icon className="h-3.5 w-3.5 text-emerald-500" />{label}</div><p className="mt-2 text-sm leading-6 text-zinc-300">{value}</p></div>;
}
